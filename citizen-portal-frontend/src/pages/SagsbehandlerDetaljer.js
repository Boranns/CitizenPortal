...
const SagsbehandlerDetaljer = () => {
  ...
  const updateStatus = async (status) => {
    try {
      ...
      setAnsogning({ ...ansogning, status: response.data.status });
    } catch (error) {
      console.error(error);
    }
  };
    ...
    <div>
      ...
      <div className="container">
        ...
        <div className="card">
          ...
          <p><strong>Borger:</strong> {ansogning.userName}</p>
          <small>Oprettet: {new Date(ansogning.createdAt).toLocaleDateString('da-DK')}</small>

          <div className="status-actions">
            <h4>Opdater status:</h4>
            <div style={{ display: 'flex', gap: '12px', marginTop: '12px' }}>
              <button className="btn btn-success" onClick={() => updateStatus('Godkendt')}>
                Godkend
              </button>
              <button className="btn btn-danger" onClick={() => updateStatus('Afvist')}>
                Afvis
              </button>
            </div>
          </div>
...
export default SagsbehandlerDetaljer;